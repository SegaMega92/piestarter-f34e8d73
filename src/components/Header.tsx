  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch(url, {
        ...
        body: JSON.stringify({ ...leadForm, source: "header_form" }),
      });
      ...
      <header className="flex items-center justify-between px-4 md:px-8 lg:px-[70px] py-[14px] md:py-[20px] w-full sticky top-0 bg-transparent backdrop-blur-sm z-50">
        ...
        <div className="hidden md:flex items-center gap-[10px] shrink-0">
          ...
          <button
            ...
            className="flex bg-p-blue items-center gap-[10px] px-[20px] lg:px-[30px] py-[14px] lg:py-[18px] rounded-[30px] cursor-pointer hover:bg-[#96d9ec] active:scale-[0.98] transition-all whitespace-nowrap"
          >
            <span className="font-medium text-[14px] lg:text-[14.9px] text-black">Оставить заявку</span>
          </button>
        </div>
        ...
        {menuOpen && (
          <div className="absolute top-full left-0 right-0 bg-bg-main/95 backdrop-blur-sm border-b border-grey-88 p-4 flex flex-col gap-3 md:hidden">
            ...
            >
              <span className="font-medium text-[14.9px] text-black">Оставить заявку</span>
            </button>
          </div>
        )}
      </header>
      ...
      <Dialog open={leadFormOpen} onOpenChange={setLeadFormOpen}>
        <DialogContent className="sm:max-w-[420px]">
          ...
          <form onSubmit={handleLeadSubmit} className="space-y-4 mt-2">
            <div>
              <label className="text-sm text-grey-44 mb-1 block">Имя *</label>
              <input
                value={leadForm.name}
            ...
            <div>
              <label className="text-sm text-grey-44 mb-1 block">Телефон *</label>
              <input
            ...
            <div>
              <label className="text-sm text-grey-44 mb-1 block">Email</label>
              <input
