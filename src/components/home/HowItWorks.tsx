...
import stepsPhoneDefault from "@/assets/home/steps-phone.png";
...
const HowItWorks = () => {
  ...
  useEffect(() => {
    supabase.from("site_settings").select("value").eq("key", "home_how_it_works").maybeSingle().then(({ data }) => {
            ...
            <div className="flex flex-col">
              {d.steps.map((step, index) => (
                <button
                  ...
                  className={`text-left transition-all duration-300 ${index === 0 ? "" : "mt-[30px]"}`}
                >
                  <div className="flex items-start gap-[8px]">
                    <span className={`font-semibold text-[28px] md:text-[40px] leading-[1] transition-colors ${activeStep === index ? "text-black" : "text-grey-71"}`}>
                      {step.number}
                    </span>
                    <h3 className={`font-semibold text-[24px] md:text-[40px] leading-[1] transition-colors ${activeStep === index ? "text-black" : "text-grey-71"}`}>
                      {step.title}
                    </h3>
                  </div>
                  <div className={`overflow-hidden transition-all duration-400 ${activeStep === index ? "max-h-[200px] opacity-100 mt-[16px]" : "max-h-0 opacity-0"}`}>
